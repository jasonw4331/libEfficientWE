...
final class CuboidTask extends ChunksChangeTask{
	...
	public function __construct(int $worldId, int $chunkX, int $chunkZ, ?Chunk $chunk, array $adjacentChunks, Vector3 $worldPos, Clipboard $clipboard, bool $fill, bool $replaceAir, \Closure $onCompletion){
			...
			if($fullBlockId !== null){
				...
				if($iterator->moveTo($ax, $ay, $az) !== SubChunkExplorerStatus::INVALID){
					...
					if($this->replaceAir || $fullBlockId !== VanillaBlocks::AIR()->getFullId()){
						...
						if($this->fill || $x === $minVector->x || $x === $maxVector->x || $y === $minVector->y || $y === $maxVector->y || $z === $minVector->z || $z === $maxVector->z){
