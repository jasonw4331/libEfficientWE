...
class Cylinder extends Shape {
	...
	private function __construct(Vector3 $relativeCenter, int $radius, int $height, ?Clipboard $clipboard = null) {
	...
	public function asyncPaste(Level $level, Vector3 $relative_pos, bool $replace_air = true, ?callable $callable = null) : void {
		...
		$task = new AsyncCylinderTask($this, $level, $chunks, AsyncCylinderTask::PASTE, $callable);
	...
	public function asyncSet(Level $level, Block $block, ?callable $callable = null) : void {
		$task = new AsyncCylinderTask($this, $level, $this->getChunks($level), AsyncCylinderTask::SET, $callable);
		$task->setBlock($block);
		$level->getServer()->getAsyncPool()->submitTask($task);
	}
	...
	public function asyncReplace(Level $level, Block $find, Block $replace, ?callable $callable = null) : void {
		$task = new AsyncCylinderTask($this, $level, $this->getChunks($level), AsyncCylinderTask::REPLACE, $callable);
		$task->find($find);
	...
	function asyncRotate(Level $level, int $direction, ?callable $callable = null) : void {
		// TODO: Implement asyncRotate() method.
	}

	function syncRotate(ChunkManager $level, int $direction, ?callable $callable = null) : void {
